...
// AJ's private companion, one single-client companion, not a course or a
// per-module family. Built as a structural sibling of mbf.js rather than a
// shared refactor of it, on purpose: mbf.js and onramp.js are live and
// serving real clients and course customers, and this file's safety logic,
// while functionally identical, needs to be independently readable and
// independently testable without risking either of those deployments.
function readPart(name) {
  return fs.readFileSync(path.join(__dirname, name), 'utf8').trim();
}
const CORE_OPEN = readPart('aj-core-open.txt');
const METHOD = readPart('aj-method.txt');
const CORE_CLOSE = readPart('aj-core-close.txt');
...
function logIncompleteResponse(fields) {
  ...
  console.error('[aj-companion] incomplete response', fields);
}
...
// deliberately separate from every other companion's access codes: AJ's
// code must never open MBF or On-Ramp, and no MBF or On-Ramp code may ever
// open this route. AJ_ACCESS_CODES is a comma-separated list (editable in
// Render without a code change); the singular AJ_ACCESS_CODE also counts,
// matching the pattern used everywhere else.
function validAccessCodes() {
  const list = String(process.env.AJ_ACCESS_CODES || '')
    .split(',')
    .map((c) => c.trim())
    .filter(Boolean);
  const single = String(process.env.AJ_ACCESS_CODE || '').trim();
  if (single) list.push(single);
  return list;
}

// Codes are issued as a client's own name (firstname-lastname), so
// normalize case and separators before comparing: AJ typing "AJ Patel" or
// "aj_patel" must still match a code Chad entered as "aj-patel" in Render.
...
(function(){
  var accessCode = '';
  try { accessCode = window.sessionStorage.getItem('ajCode') || ''; } catch (e) {}
  ...
  el('unlockButton').addEventListener('click', async function(){
    ...
    try {
      ...
      try { window.sessionStorage.setItem('ajCode', accessCode); } catch (e) {}
    } catch (error) {
      accessCode = '';
      try { window.sessionStorage.removeItem('ajCode'); } catch (e) {}
