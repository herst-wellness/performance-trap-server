...
// Chad's own thinking companion for one decision: how to answer Vivian's
// 2026-08-20 email asking whether a shorter version of the eight-session
// offer exists. Unlike every other companion in this repo, the person on
// the page is Chad himself, not a client. Built as a structural sibling of
// aj.js rather than a shared refactor, for the same reason aj.js was built
// that way: the live client-facing companions must not share mutable code
// with this page, and the safety layer needs to be independently readable
// and testable without risking those deployments.
function readPart(name) {
  return fs.readFileSync(path.join(__dirname, name), 'utf8').trim();
}
const CORE_OPEN = readPart('vivian-reply-open.txt');
const METHOD = readPart('vivian-reply-brief.txt');
const CORE_CLOSE = readPart('vivian-reply-close.txt');
const safetyOverlay = readPart('onramp-safety-overlay.txt');

const TITLE = 'The Vivian Reply';
const SUB = 'A room to think it through before you answer.';
const OPENING =
  'She said yes to the working session, and she asked her question straight: is there another version of this? Where do you want to start: what the eight sessions are for, what you read in her ask, or what you actually want here?';
const PAGE_PATH = '/practice/vivian-reply';
const API_PATH = '/api/vivian-reply';
...
const UNDECIDED_WORDS = /\b(i do not know|i don't know|not sure|torn|stuck)\b/i;
const PRICE_WORDS = /\b(discount|price|money|8,?000|8k|afford|cheaper)\b/i;
const STRUCTURE_WORDS =
  /\b(on-?ramp|shorter|smaller|another version|different structure|starting block)\b/i;
...
// One-click, user-initiated email of a sitting's own notes to Chad's inbox
// (here that means to himself), using the same Resend account already
// wired for the other companions. Page-specific to/from env vars are
// checked first, falling back to the shared COMPANION_REPORT_* vars so
// nothing new needs configuring. This never runs automatically: the
...
async function sendNotesToChad(code, transcript, fetchImpl = fetch) {
  ...
  const to = process.env.VIVIAN_REPLY_REPORT_TO || process.env.COMPANION_REPORT_TO || '';
  const from = process.env.VIVIAN_REPLY_REPORT_FROM || process.env.COMPANION_REPORT_FROM || '';
  if (!apiKey || !to || !from) {
...
function evaluateDeterministicControls({ message, adultConfirmed, country, provider }) {
  ...
  if (MEMORY_QUESTION.test(text)) {
    return result(
      'continue_reflection',
      'No. This application does not save the session or carry memory into a new one, so when you return it will not know what you wrote today. You can use the Email me this button, or copy or download it yourself.',
      false
    );
  }
  if (SAVE_OR_REMIND.test(text)) {
    return result(
      'continue_reflection',
      'This application does not create accounts, store sessions, or schedule reminders, so I cannot save this journal or remind you later. You can use the Email me this button so it lands in your inbox, or copy or download it and set a reminder in a tool you control.',
      false
    );
  }
  ...
  if (SEND_TO_CHAD.test(text)) {
    return result(
      'continue_reflection',
      'I cannot send your writing to Chad or anyone else on my own, and nothing is sent automatically. If you want it in your inbox, use the Email me this button in this session, or copy or download it and send it yourself.',
      false
    );
  }
...
function offlineReflectionResponse(message, history) {
  ...
  if (UNDECIDED_WORDS.test(message)) {
    return "That's a real answer, and the email doesn't have to carry the whole decision. She's coming to the working session either way. What would that session need to show you for this to get clear?";
  }
  if (PRICE_WORDS.test(message)) {
    return 'Before the number moves anywhere: is this about what the work costs, or about wanting to rescue her from the cost? Those write two different emails.';
  }
  if (STRUCTURE_WORDS.test(message)) {
    return 'Say the honest answer to her actual question in one sentence before you shape anything: is there another version, yes or no?';
  }
  if (userTurns <= 2) {
    return 'Before the options: what did you feel when you first read the email, before you started deciding what to do with it?';
  }
  return "You've been around this a few times now. What hasn't been said yet?";
}
...
// Same raised ceilings as the MBF and On-Ramp companions, for the same
// reason: real evaluation showed internal reasoning alone can exceed a
// 4096-token ceiling on emotionally complex moments, leaving an empty or
// truncated visible reply. This companion works at least as deep as those.
...
function logIncompleteResponse(fields) {
  ...
  console.error('[vivian-reply] incomplete response', fields);
}
...
// This is Chad's own page, gated by its own code and deliberately separate
// from every client companion's access codes: this code must never open
// MBF, On-Ramp, or AJ's page, and none of those codes may ever open this
// route. VIVIAN_REPLY_ACCESS_CODES is a comma-separated list (editable in
// Render without a code change); the singular VIVIAN_REPLY_ACCESS_CODE also
// counts, matching the pattern used everywhere else.
function validAccessCodes() {
  const list = String(process.env.VIVIAN_REPLY_ACCESS_CODES || '')
    .split(',')
    .map((c) => c.trim())
    .filter(Boolean);
  const single = String(process.env.VIVIAN_REPLY_ACCESS_CODE || '').trim();
  if (single) list.push(single);
  return list;
}

// Codes follow the firstname-lastname convention, so normalize case and
// separators before comparing: Chad typing "Chad Herst" or "chad_herst"
// must still match a code entered as "chad-herst" in Render.
...
<title>${TITLE} | Herst Wellness</title>
...
<main class="shell">
  ...
  <section id="accessCard" class="card">
    ...
    <p>Enter your code. It opens the room.</p>
    <div class="field">
  ...
  <section id="consentCard" class="card hidden">
    <h2>Welcome</h2>
    <p>This is your own room, not a client page. One decision lives here: how to answer Vivian's email. You write or speak, and it thinks with you, following what you bring rather than a fixed sequence of questions.</p>
    <p>If you would rather talk than type, press Speak and your words arrive in the box as text, yours to change before you send. It keeps nothing after you end.</p>
    <p>It knows her email, your consult with her, and the shape of your offers, and nothing else. It is not a substitute for your own read of her, and nothing written here reaches her.</p>
    <p>When you are done, the Email me this button sends what's here to your inbox. That only happens if you choose it.</p>
  ...
  <section id="session" class="session hidden">
    <div class="session-head">
      <div><div class="session-title">${TITLE}</div><div id="modeLabel" class="mode"></div></div>
      <div class="row">
        <button id="copyButton" class="button secondary">Copy</button>
        <button id="downloadButton" class="button secondary">Download</button>
        <button id="emailButton" class="button secondary">Email me this</button>
    ...
    <form id="composer" class="composer">
      <textarea id="messageInput" maxlength="12000" placeholder="Think out loud..." aria-label="Your reflection"></textarea>
    ...
    <div id="composerClosed" class="composer-closed hidden">
      ...
      <div class="row" style="justify-content:center">
        ...
        <button type="button" id="emailButtonBottom" class="button secondary">Email me this</button>
  ...
  <div class="footer">Herst Wellness &middot; This companion does not connect to the transcript database, analytics, or marketing tools. Emailing your notes to yourself only happens if you choose the Email me this button.</div>
</main>
<script>
(function(){
  var accessCode = '';
  try { accessCode = window.sessionStorage.getItem('vivianReplyCode') || ''; } catch (e) {}
  ...
  function clearSession(){
    ...
    showComposer();
    try { window.sessionStorage.removeItem('vivianReplyCode'); } catch (e) {}
    setLocked(false);
  }
  ...
  el('unlockButton').addEventListener('click', async function(){
    ...
    try {
      var response = await fetch('${API_PATH}', {headers:{'X-Companion-Access':accessCode}, cache:'no-store'});
      ...
      try { window.sessionStorage.setItem('vivianReplyCode', accessCode); } catch (e) {}
    } catch (error) {
      accessCode = '';
      try { window.sessionStorage.removeItem('vivianReplyCode'); } catch (e) {}
  ...
  function enterSession(){
    ...
    addMessage('assistant', ${JSON.stringify(OPENING)});
    el('messageInput').focus();
  }
  el('beginButton').addEventListener('click', enterSession);
  ...
  async function sendForTranscription(){
    ...
    try {
      var response = await fetch('/api/vivian-reply/transcribe', {
        method: 'POST',
  ...
  function doDownload(){
    ...
    a.href = url; a.download = 'vivian-reply-notes.txt'; a.click();
  ...
  async function doEmail(btn){
    ...
    try {
      ...
      status.textContent = 'Sent to your inbox.';
    } catch (error) {
...
const TRANSCRIBE_PATH = '/api/vivian-reply/transcribe';
...
async function transcribeAudio(bytes, contentType) {
  ...
  const model = process.env.VIVIAN_REPLY_TRANSCRIBE_MODEL || process.env.ONRAMP_TRANSCRIBE_MODEL || 'whisper-1';
  ...
  if (!response.ok) {
    ...
    console.error('[vivian-reply] transcription failed', { status: response.status, model, detail });
...
async function handleVivianReplyRoute(req, res) {
