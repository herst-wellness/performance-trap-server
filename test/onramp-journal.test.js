...
test('the journal page carries its own copy, no breath card, the journal card, and is not listed as a daily rep', { timeout: 30000 }, async (t) => {
  ...
  const parts = (a, b) => [
    ...
    { title: 'One Moment, Mapped', text: b },
  ];
  ...
  assert.deepEqual(j.journals.map((x) => x.key), ['week-1/whats-bringing-you-here', 'week-1/one-moment-mapped']);
  ...
  assert.deepEqual(onramp.journalTitlesOf(WEEK_TURN), ["What's Bringing You Here", 'One Moment, Mapped']);
  assert.deepEqual(onramp.journalTitlesOf(FIRST_TURN), []);
  assert.deepEqual(onramp.journalSessionFor(j, WEEK_TURN), { key: 'week-1', journalTitles: ["What's Bringing You Here", 'One Moment, Mapped'] });
...
test('consent round-trips from the lesson page, and with consent the journal exchange is kept on the record', { timeout: 30000 }, async (t) => {
  ...
  assert.deepEqual(record.journalSessions[key].journalTitles, ["What's Bringing You Here", 'One Moment, Mapped'], 'the titles come from the headings');
  ...
  assert.deepEqual(record.journalSessions[key].journalTitles, ['One Moment, Mapped']);
  ...
  assert.deepEqual(record.journalSessions[key].journalTitles, ["What's Bringing You Here", 'One Moment, Mapped']);
...
test('the Week 1 lesson carries the consent box and a Bring it link per journal; other weeks do not', () => {
  ...
  const w4 = lessonContentHtml(4);
  for (const key of ['week-4/whats-still-running', 'week-4/what-youre-taking-with-you']) {
    assert.ok(w4.includes('href="/practice/on-ramp/journal-4?journal=' + key + '">Bring it to the journal sitting</a>'), 'Bring it link for ' + key);
  }
  assert.ok(!w4.includes('journalConsent'), 'the consent box lives on Week 1 only');
});
...
test('buildBriefInput carries the name, the practice numbers by week, each saved sitting as writing plus exchange', () => {
  ...
  record.journalSessions = {
    'week-1': {
      ...
      journalTitles: ["What's Bringing You Here", 'One Moment, Mapped'],
      history: [
        { role: 'user', content: WEEK_TURN },
        { role: 'assistant', content: STUB_TEXT },
        { role: 'user', content: 'It gets tighter.' },
  ...
  assert.deepEqual(brief.splitWeekTurn(WEEK_TURN), { week: 1, journals: [{ title: "What's Bringing You Here", text: JOURNAL_TEXT }, { title: 'One Moment, Mapped', text: BREATH_TEXT }] });
...
test('the admin brief route needs the admin code, builds the input from the record, and emails Chad through Resend', { timeout: 30000 }, async (t) => {
  ...
  assert.ok(input.includes('[One Moment, Mapped]\n' + BREATH_TEXT));
...
test('a journal that contains "I\'m done." or "stop" is writing, not a stop request; the safety routes still fire on it', () => {
  ...
  assert.equal(evaluateDeterministicControls({ ...base, message: "Journal: One Moment, Mapped\n\nI'm done. I told her to stop.", journalText: true }), null);
...
test('the Week 1 lens (Slow the breath, Enter the body) sits in both the daily rep and the journal sitting, and in no other week', () => {
  ...
  const lens4 = "The lens this week: What's Underneath, The Whole Arc, What Comes Next";
  assert.ok(WEEKS[4].instructions.includes(lens4), 'Week 4 daily rep carries the Week 4 lens');
  assert.ok(onramp.JOURNAL[4] && onramp.JOURNAL[4].instructions.includes(lens4), 'the Week 4 journal sitting carries the Week 4 lens');
  assert.deepEqual(onramp.JOURNAL[4].journals.map((x) => x.key), ['week-4/whats-still-running', 'week-4/what-youre-taking-with-you']);
  for (const n of [1, 2, 3]) assert.ok(!WEEKS[n].instructions.includes(lens4), 'week ' + n + ' has no Week 4 lens');
