...
// Lorenzo's private companion, one single-client companion, not a course or
// a per-module family. Built as a structural sibling of aj.js rather than a
// shared refactor of it, on purpose: aj.js, mbf.js, and onramp.js are live
// and serving real clients and course customers, and this file's safety
// logic, while functionally identical, needs to be independently readable
// and independently testable without risking any of those deployments.
function readPart(name) {
  return fs.readFileSync(path.join(__dirname, name), 'utf8').trim();
}
const CORE_OPEN = readPart('lorenzo-core-open.txt');
const METHOD = readPart('lorenzo-method.txt');
const CORE_CLOSE = readPart('lorenzo-core-close.txt');
const safetyOverlay = readPart('onramp-safety-overlay.txt');

const TITLE = 'Lorenzo’s Companion';
const SUB = 'Between sessions with Chad.';
const OPENING = 'Ciao, Lorenzo. What’s in front of you right now? A yes that’s forming somewhere, the back, the rest. Start anywhere.';
const PAGE_PATH = '/practice/lorenzo';
const API_PATH = '/api/lorenzo';
...
// (kids-on-the-bus/lib/weekly-report.js). Lorenzo-specific to/from env vars
// are checked first so this can be pointed elsewhere without touching that
...
async function sendNotesToChad(code, transcript, fetchImpl = fetch) {
  ...
  const to = process.env.LORENZO_REPORT_TO || process.env.COMPANION_REPORT_TO || '';
  const from = process.env.LORENZO_REPORT_FROM || process.env.COMPANION_REPORT_FROM || '';
  if (!apiKey || !to || !from) {
    const err = new Error('Email is not configured.');
    err.clientStatus = 503;
    throw err;
  }
  const subject = 'Lorenzo companion notes - ' + code;
...
function logIncompleteResponse(fields) {
  ...
  console.error('[lorenzo-companion] incomplete response', fields);
}
...
// This is a single, named client's companion, enrolled manually by Chad and
// deliberately separate from every other companion's access codes:
// Lorenzo's code must never open AJ, MBF, or On-Ramp, and no AJ, MBF, or
// On-Ramp code may ever open this route. LORENZO_ACCESS_CODES is a
// comma-separated list (editable in Render without a code change); the
// singular LORENZO_ACCESS_CODE also counts, matching the pattern used
// everywhere else.
function validAccessCodes() {
  const list = String(process.env.LORENZO_ACCESS_CODES || '')
    .split(',')
    .map((c) => c.trim())
    .filter(Boolean);
  const single = String(process.env.LORENZO_ACCESS_CODE || '').trim();
  if (single) list.push(single);
  return list;
}

// Codes are issued as a client's own name (firstname-lastname), so
// normalize case and separators before comparing: Lorenzo typing
// "Lorenzo Lastname" or "lorenzo_lastname" must still match a code Chad
// entered as "lorenzo-lastname" in Render.
...
<main class="shell">
  ...
  <section id="accessCard" class="card">
    ...
    <p>Enter the code Chad gave you. It opens your companion.</p>
    <div class="field">
  ...
  <section id="consentCard" class="card hidden">
    ...
    <p>If you would rather talk than type, you can still record and speak into it, and press send, and then it will respond in writing. Your words arrive in the box as text first, yours to change before you send. It keeps nothing after you end.</p>
...
(function(){
  var accessCode = '';
  try { accessCode = window.sessionStorage.getItem('lorenzoCode') || ''; } catch (e) {}
  ...
  function clearSession(){
    ...
    showComposer();
    try { window.sessionStorage.removeItem('lorenzoCode'); } catch (e) {}
    setLocked(false);
  }
  ...
  el('unlockButton').addEventListener('click', async function(){
    ...
    try {
      ...
      try { window.sessionStorage.setItem('lorenzoCode', accessCode); } catch (e) {}
    } catch (error) {
      accessCode = '';
      try { window.sessionStorage.removeItem('lorenzoCode'); } catch (e) {}
  ...
  async function sendForTranscription(){
    ...
    try {
      var response = await fetch('/api/lorenzo/transcribe', {
        method: 'POST',
  ...
  function doDownload(){
    ...
    a.href = url; a.download = 'lorenzo-companion-notes.txt'; a.click();
...
const TRANSCRIBE_PATH = '/api/lorenzo/transcribe';
...
async function transcribeAudio(bytes, contentType) {
  ...
  const model = process.env.LORENZO_TRANSCRIBE_MODEL || process.env.ONRAMP_TRANSCRIBE_MODEL || 'whisper-1';
  ...
  if (!response.ok) {
    ...
    console.error('[lorenzo-companion] transcription failed', { status: response.status, model, detail });
...
async function handleLorenzoRoute(req, res) {
